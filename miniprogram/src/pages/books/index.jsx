...
const STATUS_LABEL = {
  reading:  { label: '在读', color: '#1e6db8', bg: '#eaf2fb' },
  want:     { label: '想读', color: '#888',    bg: '#f5f5f5' },
  finished: { label: '读完', color: '#3a7d52', bg: '#eef5f1' }
}
...
export default function BooksPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setBooks((data.books || []).filter(b => b.source === 'weread'))
    } catch {
  ...
  // 想读：其余
  const readingAllIds = new Set(books.filter(b => !finishedIds.has(b.id) && b.status === 'reading').map(b => b.id))
  const readingTopIds = new Set(reading.map(b => b.id))
  const want = books.filter(b => !finishedIds.has(b.id) && (!readingAllIds.has(b.id) || !readingTopIds.has(b.id)))
