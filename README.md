...
| 层 | 技术 |
|---|---|
| 云端 API / 定时任务 | Cloudflare Workers + Workers KV |
| 微信小程序 | Taro（React 语法，编译到微信小程序） |
| 网页前端 | Vue.js（内联在单个 HTML 文件中） |
| 本地服务器 | Python Flask（纯标准库，无框架依赖） |
| 数据同步 | Python 脚本 + GitHub Actions + Cloudflare Cron |
| 微信读书接入 | 微信读书 API Gateway（第三方，wasm 解密 + 请求签名） |
...
| 入口 | 地址 | 数据写哪里 |
|---|---|---|
| 🌐 云端网页（推荐） | `https://yangminggu.com/tasks` | **直接写 Cloudflare Worker KV**（云端） |
| 📱 微信小程序 | 微信开发者工具 / 正式版 | **直接写 Cloudflare Worker KV**（云端） |
| 💻 本地网页（开发用） | `http://localhost:8080` | **只写本地 `data/`**，不推云端 |

> `web/dashboard.html` 同时被本地 Flask（localhost:8080）和 Cloudflare Worker（yangminggu.com/tasks）托管，是同一个文件。
> 区别在于 API 调用对象不同：本地版打本地 Flask API，云端版打 Worker API（直接写 KV）。
