...
class WeReadStoreTest(unittest.TestCase):
    def test_merge_weread_store_preserves_existing_book_id(self):
        ...
        self.assertEqual(merged["books"][0]["id"], 12)
    ...
    def test_merge_weread_store_preserves_existing_note_id(self):
        merged = merge_weread_store(
            {"notes": [{"id": 9, "_bookId": "book-1", "title": "旧笔记", "summary": "旧"}]},
            {"notes": [{"_bookId": "book-1", "title": "旧笔记", "summary": "新"}]},
        )

        self.assertEqual(len(merged["notes"]), 1)
        self.assertEqual(merged["notes"][0]["id"], 9)
    ...
    def test_merge_weread_notes_store_deduplicates_by_source_item_id(self):
        ...
        self.assertEqual(len(merged["notes"]), 1)
        self.assertEqual(merged["notes"][0]["id"], 5)
        self.assertEqual(merged["notes"][0]["title"], "新笔记")
        self.assertEqual(merged["notes"][0]["summary"], "新")

    def test_merge_weread_notes_store_keeps_remaining_existing_note_id(self):
        merged = merge_weread_notes_store(
            {
                "notes": [
                    {"id": 5, "sourceItemId": "existing", "title": "保留"},
                    {"id": 6, "sourceItemId": "updated", "title": "旧"},
                ]
            },
            {"notes": [{"sourceItemId": "updated", "title": "新"}]},
        )

        by_source = {note["sourceItemId"]: note for note in merged["notes"]}
        self.assertEqual(by_source["existing"]["id"], 5)
        self.assertEqual(by_source["updated"]["id"], 6)
